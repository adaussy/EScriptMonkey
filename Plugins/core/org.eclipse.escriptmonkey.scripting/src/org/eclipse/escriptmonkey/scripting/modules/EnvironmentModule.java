...
public class EnvironmentModule extends AbstractScriptModule implements IScriptModule {
	...
	public final IScriptModule loadModule(final String moduleIdentifier) {
		...
		if(module == null) {
			// not yet loaded
			final Map<String, ModuleDefinition> availableModules = ScriptService.getInstance().getAvailableModules();

			final ModuleDefinition definition = availableModules.get(moduleIdentifier);
			if(definition != null) {
				// module exists

				// load dependencies
				for(final String dependencyName : definition.getDependencies()) {
	...
	private void createWrappers(final IScriptModule module, final boolean reload) {
		...
		final StringBuffer scriptCode = new StringBuffer();

		// use reflection to access methods
		for(final Method method : module.getClass().getMethods()) {
			if(useAutoLoader(method)) {

				final String preExecutionCode = getPreExecutionCode(method);
				final String postExecutionCode = getPostExecutionCode(method);
				...
				final String code = getWrapper().createFunctionWrapper(getRegisteredModuleName(module.getModuleName()), method, methodNames, IScriptFunctionModifier.RESULT_NAME, preExecutionCode, postExecutionCode);
		...
		if(!reload) {
			final Field[] declaredFields = module.getClass().getDeclaredFields();
		...
		if(filename.contains("://")) {
			...
			if(filename.startsWith(PROJECT)) {
				...
				if(currentFile instanceof IFile) {
					final IFile file = ((IFile)currentFile).getProject().getFile(new Path(filename.substring(PROJECT.length())));
					if(file.exists())
				...
				final IFile workspaceFile = ResourcesPlugin.getWorkspace().getRoot().getFile(new Path(filename.substring(WORKSPACE.length())));
	...
	private void fireModuleEvent(final IScriptModule module, final int type) {
		for(final Object listener : mModuleListeners.getListeners())
	...
	void addModule(final IScriptModule module) {
		...
		for(final IScriptModule mod : mModules) {

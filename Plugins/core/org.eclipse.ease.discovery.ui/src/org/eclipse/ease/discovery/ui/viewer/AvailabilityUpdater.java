...
public class AvailabilityUpdater {
					...
					for(Future<VerifyUpdateSiteJob> jobFuture : futures) {
						try {
							...
							if(e.getCause() instanceof CoreException) {
								...
								status = new Status(IStatus.ERROR, Activator.PLUGIN_ID, "Error while checking update sites availability.", e.getCause());
							}
							Logger.logError(status.getMessage(), status.getException());
						}
						monitor.worked(1);
					}
				} finally {
	...
	private void handleSiteURL(Map<String, Collection<InstallableComponent>> urlToDescriptors, InstallableComponent descriptor, String url) {
